...
class any final {
 ...
 protected:
  ...
  template <typename T>
  friend const T* dy_cast(const any* operand) noexcept;
  template <typename T>
  friend T* dy_cast(any* operand) noexcept;
  ...
  template <typename T>
  const T* dy_cast() const noexcept {
    return requires_allocation<typename std::decay<T>::type>::value
               ? dynamic_cast<const T*>(storage.dynamic)
               : dynamic_cast<const T*>(&storage.stack);
  }

  template <typename T>
  T* dy_cast() noexcept {
    return requires_allocation<typename std::decay<T>::type>::value
               ? dynamic_cast<T*>(storage.dynamic)
               : dynamic_cast<T*>(&storage.stack);
  }
...
template <typename ValueType>
inline ValueType dy_cast(any& operand) {
  auto p = dy_cast<typename std::remove_reference<ValueType>::type>(&operand);
#ifndef ANY_IMPL_NO_EXCEPTIONS
  if (p == nullptr) throw bad_any_cast();
#endif
  return *p;
...
template <typename ValueType>
inline ValueType* dy_cast(any* operand) noexcept {
  using T = typename std::decay<ValueType>::type;
  return operand->dy_cast<ValueType>();
  //return nullptr;
}
...
namespace std {
inline void swap(linb::any& lhs, linb::any& rhs) noexcept { lhs.swap(rhs); }
}  // namespace std